import styles from './layout.module.css'
import Head from 'next/head'
import Image from 'next/image'
import utilStyles from '../styles/utils.module.scss'
import Link from 'next/link'

import Cursor from '../pages/cursor/Cursor'
import cursor_styles from '../pages/cursor/cursor.module.scss'

const name = 'Kate'
export const siteTitle = 'Next.js Sample Website'

export default function Layout({children, home}){
    return (
        <Cursor
            hoverClasses={[
                { classNameOfTargetElement: "link", classNameOfStyle: cursor_styles.cursor_hover},
                { classNameOfTargetElement: "video", classNameOfStyle: cursor_styles.cursor_video},
                { classNameOfTargetElement: "video_preview", classNameOfStyle: cursor_styles.cursor_videoPreview},
                { classNameOfTargetElement: utilStyles.dragg, classNameOfStyle: cursor_styles.cursor_drag},

                utilStyles.dragg
            ]}>
            <div className={styles.container}>
                    <Head>
                        <link rel="icon" href="/favicon.ico" />
                        <meta name="description" content="Learn how to build a personal website using Next.js" />
                        <meta property="og:image" content={`https://og-image.vercel.app/${encodeURI(siteTitle)}.png?theme=light&md=0&fontSize=75px&images=https%3A%2F%2Fassets.vercel.com%2Fimage%2Fupload%2Ffront%2Fassets%2Fdesign%2Fnextjs-black-logo.svg`}/>
                    </Head>
                    <header className={styles.header}>
                        {home ? (
                            <>
                                <Image priority src="/images/profile.jpg" className={utilStyles.borderCircle} height={144} width={144} alt={name} />
                                <h1 className={utilStyles.heading2X1}> {name} </h1>
                            </>
                        ) : (
                            <>
                                <Link href="/">
                                    <a>
                                        <Image priority src="/images/profile.jpg" className={utilStyles.borderCircle} height={108} width={108} alt={name} />
                                    </a>
                                </Link>
                                <h2 className={utilStyles.headingLg}>
                                    <Link href="/">
                                        <a className={utilStyles.colorInherit}>{name}</a>
                                    </Link>
                                </h2>
                            </>
                        )}
                    </header>
                    <main>{children}</main>
                    {!home && (
                        <div className={`${styles.backToHome} link`}>
                            <Link href="/">
                                <a>Back to home</a>
                            </Link>
                        </div>
                    )}
                    <div>

                    </div>
            </div>
        </Cursor>
    )
}